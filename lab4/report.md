...
##### Потенциально полезные индексы

Для того чтобы ускорить `Н_ВЕДОМОСТИ.ИД < 1490007` корректно использовать B-tree индекс для поля `ИД`, 
так как B-tree имеет сложность подобного поиска O(logN), лучше только O(1), но в БД так не принято - 
терям в памяти при любой реализаа, а современные реляционные БД не приветсвуют создание подобных конструкий 
или вовсе не предоставляют таких возможностей.

Для того чтобы ускорить `Н_ЛЮДИ.ИМЯ = 'Ярослав'` можно использовать B-tree или Hash-индекс, 
но Hash в данном случае предпочтительнее, так как с подобным полем используется =, а не операторы сравнения. 
Также логично предлопожить, что имена редко приходится сравнивать друг с другом, чаще имя выбирается по значению.
...
<img src="images/second_query_var1.png" width="340" alt="план запроса 2 без индексов, неэффективный">

<img src="images/second_query_var2.png" width="340" alt="план запроса 2 без индексов, эффективный">
...
Во всех случаях используется операция сравнения, следовательно 
лучше использовать B-tree индексы для каждого из полей отношений, используемых в выборке `where`.
...
В ходе выполнения лабораторной работы я изучил планы выполнения sql-запросов, 
индексы и варианты оптимизации плана запроса.